 ...
 */
export function queryNotes(db, filters, orderBy, limit, offset) {
 ...
 */
function getRecencyBoost(modifiedISO) {
    const now = new Date();
 ...
 */
export function combinedSearch(db, query, filters, limit, offset) {
    // Run FTS first
    ...
    const modifiedMap = new Map(filteredRows.map((row) => [row.note_id, row.modified]));
