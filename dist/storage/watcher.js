...
export class AnvilWatcher {
    ...
    handleEvent(eventType, filePath) {
        ...
        if (this.debounceTimer)
            clearTimeout(this.debounceTimer);
        this.debounceTimer = setTimeout(() => this.processBatch(), this.options.debounceMs ?? 500);
    }
    /**
    ...
    async startupCatchup() {
        const indexedPaths = new Map(getAllNotePaths(this.options.db).map((row) => [row.filePath, row.modified]));
        const toReindex = [];
        const currentPaths = new Set();
        // Walk filesystem
        for await (const file of scanVault(this.options.vaultPath, this.buildIgnorePatternsArray())) {
