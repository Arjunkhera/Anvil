 ...
 */
export async function handleUpdateNote(input, ctx) {
    try {
        ...
        if (!type) {
            return makeError(ERROR_CODES.TYPE_NOT_FOUND, `Type not found: ${existingNote.type}`);
        }
        ...
        if (type.behaviors.append_only &&
            ...
            if (!existingNote.body.includes(input.content)) {
                return makeError(ERROR_CODES.APPEND_ONLY, `Type ${existingNote.type} has append_only behavior: body can only be appended, not replaced`);
            }
        }
            ...
            for (const fieldName of immutableFields) {
                if (fieldName in input.fields) {
                    ...
                    if (immutableDef?.immutable) {
                        ...
                        const oldValue = existingNote[fieldName];
                        if (newValue !== oldValue) {
                            return makeError(ERROR_CODES.IMMUTABLE_FIELD, `Field is immutable and cannot be changed: ${fieldName}`);
                        }
                    }
                }
            }
        ...
        if (input.fields) {
            ...
            for (const [key, value] of Object.entries(input.fields)) {
                if (['title', 'status', 'priority', 'due', 'effort', 'tags', 'related', 'scope'].includes(key)) {
        ...
        if (!validation.valid) {
            ...
            return makeError(ERROR_CODES.VALIDATION_ERROR, `Note validation failed`, { fields: fieldErrors });
        }
        ...
        try {
            ...
            return makeError(ERROR_CODES.IO_ERROR, `Failed to write note: ${err instanceof Error ? err.message : String(err)}`);
        }
        // 8. Re-index note
        try {
            ...
            return makeError(ERROR_CODES.SERVER_ERROR, `Failed to update index: ${err instanceof Error ? err.message : String(err)}`);
        }
        ...
        return makeError(ERROR_CODES.SERVER_ERROR, `Unexpected error updating note: ${err instanceof Error ? err.message : String(err)}`);
    }
}
