 ...
 */
export async function handleCreateNote(input, ctx) {
    try {
        // 1. Validate type exists in registry
        if (!ctx.registry.hasType(input.type)) {
            return makeError(ERROR_CODES.TYPE_NOT_FOUND, `Type not found: ${input.type}`);
        }
        ...
        const note = {
            ...
            due: frontmatter.due,
            effort: typeof frontmatter.effort === 'number' ? frontmatter.effort : undefined,
        ...
        try {
            ...
            return makeError(ERROR_CODES.IO_ERROR, `Failed to write note to filesystem: ${err instanceof Error ? err.message : String(err)}`);
        }
        ...
        try {
            ...
            return makeError(ERROR_CODES.SERVER_ERROR, `Failed to index note: ${err instanceof Error ? err.message : String(err)}`);
        }
        // 10. Return result
        return {
            noteId,
        ...
        return makeError(ERROR_CODES.SERVER_ERROR, `Unexpected error creating note: ${err instanceof Error ? err.message : String(err)}`);
    }
}
/**
 ...
 */
function extractTypeSpecificFields(frontmatter, type) {
