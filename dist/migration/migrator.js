...
import { inferType } from './type-inferrer.js';
import { extractDataviewFields, convertDataviewFields } from './dataview-converter.js';
import { createEmptyReport, addFileResult, } from './report.js';
/**
 ...
 * 9. Return report
 */
export async function migrate(config, db) {
    ...
    for (const file of files) {
        const result = await processFile(file.fullPath, file.filePath, config);
 ...
 */
async function processFile(fullPath, relativeFilePath, config) {
    const result = {
    ...
    try {
        ...
        if (dataviewFields.length > 0) {
            const { newBody: cleanedBody, convertedFields } = convertDataviewFields(body, dataviewFields);
            newBody = cleanedBody;
 ...
 */
async function backupFile(vaultPath, relativeFilePath, fileContent) {
