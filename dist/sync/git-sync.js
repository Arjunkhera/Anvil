 ...
 */
export async function isGitRepo(vaultPath) {
    try {
 ...
 */
export async function syncPull(vaultPath, remote = 'origin', branch, watcher) {
    try {
        ...
        try {
            ...
            return makeError(ERROR_CODES.SYNC_ERROR, `Failed to fetch from remote "${remote}": ${err instanceof Error ? err.message : String(err)}`);
        }
        ...
        try {
            ...
            try {
                ...
                return makeError(ERROR_CODES.SYNC_ERROR, `Failed to merge from "${mergeRef}": ${err instanceof Error ? err.message : String(err)}`);
            }
        }
 ...
 */
export async function syncPush(vaultPath, message) {
    try {
        // Check if it's a git repo
        if (!(await isGitRepo(vaultPath))) {
            return makeError(ERROR_CODES.NO_GIT_REPO, 'Vault is not a Git repository. Initialize git with "git init".');
        ...
        try {
            ...
            return makeError(ERROR_CODES.SYNC_ERROR, `Failed to stage files: ${err instanceof Error ? err.message : String(err)}`);
        }
        ...
        try {
            ...
            return makeError(ERROR_CODES.SYNC_ERROR, `Failed to commit: ${err instanceof Error ? err.message : String(err)}`);
        }
        // 5. Push
        try {
            await git.push();
            ...
            if (errMsg.includes('no remote') || errMsg.includes('No remote')) {
                return makeError(ERROR_CODES.NO_REMOTE, 'No remote configured for this repository. Add a remote with "git remote add origin <url>".');
            }
            return {
                status: 'push_failed',
                message: errMsg,
 ...
 */
async function detectConflictMarkers(vaultPath) {
    const git = simpleGit(vaultPath);
    const status = await git.status();
    const conflictFiles = [];
    ...
    if (status.modified)
        filesToCheck.push(...status.modified);
    if (status.created)
        filesToCheck.push(...status.created);
